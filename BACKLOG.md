...
| # | Título | Descripción | Esfuerzo |
|---|--------|-------------|----------|
| B11 | Voseo argentino en bajada del link compartido | `src/app/(site)/app/page.tsx:20` usa "Escuchá este artículo..." (voseo). Cambiar a español neutro, ej. "Escucha este artículo como podcast en Audiodocs." | XS |
| B13 | Artwork pixelado en Now Playing / lock screen | `AudioPlayerContext.tsx:649-653` deja `artwork: undefined` cuando `playingArticle.imageUrl` no existe (frecuente: no todos los artículos scrapeados traen imagen; también pasa durante el jingle si el artículo aún no confirmó imagen). Sin fallback, iOS/Safari usa su propio candidato (probablemente el favicon de 32x32) y lo estira al tile grande del Now Playing, quedando pixelado. Fix: cuando no haya `imageUrl`, pasar explícitamente `/android-chrome-512x512.png` y `/android-chrome-192x192.png` (ya existen en `public/`) como `artwork` con sus `sizes` declarados, en vez de dejarlo `undefined` | XS |
| B12 | AbortError falso en play() del reproductor | Race condition en `AudioPlayerContext.tsx` (`setupAndPlay:354-381`): la promesa de `.play()` no se rastrea ni cancela, así que al saltar párrafo/artículo, cambiar voz/engine, o pausar justo tras dar play (`playArticle:441-446`, `handleStop:562-565`, `handleEngineChange:605-609`, `handlePlayPause:524`), el navegador la aborta y dispara el mensaje "play() bloqueado [AbortError]" aunque la reproducción nueva siga bien — confuso pero cosmético. Fix: rastrear la promesa de play() en vuelo y esperarla/ignorar su AbortError esperado antes de mutar `src`/`pause()` de la siguiente reproducción, mismo patrón que B2 pero para la promesa en vez del elemento `&lt;audio&gt;`. De paso, agregar `.catch` faltante en la rama "reanudar" de `handlePlayPause` (línea 531, hoy sin manejo de rechazo) | S |
...
| F16 | Acortar link de artículo compartido | Hoy `handleShare` (`src/app/(site)/app/articles/[id]/page.tsx:67-81`) arma el deep link con `url` + `ogTitle` + `ogImage` como query params sin comprimir → 400-600+ caracteres típicos. Pasar el link generado por un servicio externo de acortamiento (TinyURL/is.gd) antes de copiarlo/compartirlo. Debe seguir resolviendo a la misma URL con los params intactos para no romper la vista previa dinámica de B10 (`generateMetadata` en `page.tsx:13-37`) — validar que el crawler de cada red social siga el redirect del shortener y lea el OG de la página real, no una interstitial del shortener | S |
