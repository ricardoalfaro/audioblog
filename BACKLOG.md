...
| F7 | i18n de la interfaz | El usuario selecciona el idioma de la UI desde el dropdown del avatar (`HeaderActions.tsx`, junto al selector de Tema). Idiomas soportados: Español (default), Inglés, Portugués, Francés y Alemán. Preferencia guardada en `localStorage`. Sin routing por segmento de idioma (resuelve con context/provider client-side). **Reestimado M→L**: no hay librería i18n instalada, se parte de cero; diagnóstico ~150-220 strings hardcodeados en ~13-15 archivos existentes (grueso en `AppClient.tsx` 847 líneas y el reader 475 líneas, con varios strings dinámicos por template literal, no extracción mecánica) + 6-7 archivos nuevos (context, diccionarios por idioma, hook) | L |
